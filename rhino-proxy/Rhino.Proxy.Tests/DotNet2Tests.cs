...
namespace Castle.DynamicProxy.Test
{
    ...
    public class DotNet2Tests
    {
    	...
    	[Test]
    	public void ProxyWithGenericMethods()
    	{
			ProxyGenerator pg = new ProxyGenerator();
			LogInvocationInterceptor logger = new LogInvocationInterceptor();
			WithGenericMethods x = (WithGenericMethods)pg.CreateClassProxy(typeof(WithGenericMethods),
				logger);
			x.Test<int>();
			x.Test<string>();
			Assert.AreEqual("Test", logger.Invocations[0]);
    		Assert.AreEqual(typeof(int), logger.Types[0]);
			Assert.AreEqual("Test", logger.Invocations[1]);
			Assert.AreEqual(typeof(string), logger.Types[1]);
