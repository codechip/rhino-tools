...
namespace Rhino.Proxy.Serialization
{
	using System;
	...
	{
		private static CodeDOMProxyBuilder builder = new CodeDOMProxyBuilder();
		...
		{
			builder = new CodeDOMProxyBuilder();
		}
		...
		public object RecreateInterfaceProxy(SerializationInfo info, StreamingContext context)
		{
			...
			if (_mixins.Length == 0)
			{
				Type proxy_type = builder.CreateInterfaceProxy(target.GetType(), _interfaces);

				proxy = Activator.CreateInstance( proxy_type, new object[] { _interceptor, target } );
			}
			else
			{
				Type proxy_type = builder.CreateCustomInterfaceProxy(_interfaces, target.GetType(), genContext);
		...
		public object RecreateClassProxy(SerializationInfo info, StreamingContext context)
		{
			...
			if (_mixins.Length == 0)
			{
				proxy_type = builder.CreateProxy(_baseType, _interfaces);
			}
			else
			{
				proxy_type = builder.CreateCustomClassProxy(_baseType, _interfaces, genContext);
			}
