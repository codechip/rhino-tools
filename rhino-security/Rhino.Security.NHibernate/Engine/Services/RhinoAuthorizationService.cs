...
namespace Rhino.Security.Engine.Services
{
  ...
  /// queries
  /// </summary>
  public class RhinoAuthorizationService<TPermission> : IAuthorizationService
  {
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="RhinoAuthorizationService{TPermission}"/> class.
    /// </summary>
    ...
    public RhinoAuthorizationService(IPermissionsService permissionsService,
    ...
    {
      IPermission[] permissions = permissionsService.GetPermissionsFor(user, entity, operation);
    ...
    {
      IPermission[] permissions = permissionsService.GetPermissionsFor(user, operation);
      if (permissions.Length == 0)
        return false;
      return permissions[0].Allow;
    }
    ...
    private ICriterion GetPermissionQueryInternal(IUser user, string operation, string securityKeyProperty)
    {
      ...
      string[] operationNames = SecurityStringHelper.GetHierarchicalOperationNames(operation);
      DetachedCriteria criteria = DetachedCriteria.For<TPermission>("permission")
    ...
    private void AddPermissionDescriptionToAuthorizationInformation<TEntity>(string operation,
                                                                             AuthorizationInformation info,
                                                                             IUser user, IPermission[] permissions,
                                                                             TEntity entity)
      where TEntity : class
    {
      ...
      if (entity != null)
      {
        ...
        entitiesGroupsDescription = SecurityStringHelper.Join(entitiesGroups);
      }
      if (permissions.Length == 0)
      {
        ...
        if (entity == null) //not on specific entity
        {
          info.AddDeny(Resources.PermissionForOperationNotGrantedToUser,
                       ...
                       SecurityStringHelper.Join(usersGroups)
            );
        }
        else
        {
    ...
    private bool InitializeAuthorizationInfo(string operation, out AuthorizationInformation info)
    {
      ...
      IOperation op = authorizationRepository.GetOperationByName(operation);
      if (op == null)
      {
    ...
    private void AddUserGroupLevelPermissionMessage(string operation, AuthorizationInformation info,
                                                    IUser user, IPermission permission,
                                                    string entityDescription,
                                                    string entitiesGroupsDescription)
    {
      if (permission.UsersGroup != null)
      {
        IUsersGroup[] ancestryAssociation =
          authorizationRepository.GetAncestryAssociation(user, permission.UsersGroup.Name);
        string groupAncestry = SecurityStringHelper.Join(ancestryAssociation, " -> ");
        if (permission.Allow)
        {
    ...
    private static string GetPermissionTarget(IPermission permission, string entityDescription,
                                              string entitiesGroupsDescription)
    {
